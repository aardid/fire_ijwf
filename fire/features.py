...
from fire.data import WeatherData
from fire.utilities import datetimeify, load_dataframe, save_dataframe, _is_fire_in, makedir
# from .model import MultiStationForecastModel
...
This objects just manipulafis feature matrices that already exist. 

Todo:
- method to mark rows with imcomplete data 
- check sensitivity to random number seed for noise mirror (FeaturesMulti._load_fis())
- criterias for feature selection (see FeaturesSta.reduce())
'''

class FeaturesSta(object):
    """ Class to loas manipulate feature matrices derived from weather data
    ...
    """
    def __init__(self, station='SCA', window=2., datastream='zsc2_T', feat_dir=None, ti=None, tf=None, 
        	fis_dir=None, dt=None, lab_lb=2.):
        ...
        self.colm_keep=None
        self._load_fis() # create self.fis (and self._fis_mirror)
        self.load() # create dataframe from feature matrices
    def _load_fis(self):
        ''' Load firetive times, and create atribute self.fis
            Parameters:
            -----------
            fis_dir : str
                Repository location on firetive times file
        ...
        '''
        # get fires
        fl_nm=os.sep.join([self.fis_dir,self.station+'_fire_ignitions.txt'])
        with open(fl_nm,'r') as fp:
            self.fis=[datetimeify(ln.rstrip()) for ln in fp.readlines()]
    def load(self, drop_nan=True):
        ...
        ts=fM.index.values
        ys=[_is_fire_in(days=self.lab_lb, from_time=t, fis=self.fis) for t in pd.to_datetime(ts)]
...
class FeaturesMulti(object):
    ...
    """
    def __init__(self, stations=None, window=2., datastream='zsc2_T', feat_dir=None, 
        dtb=None, dtf=None, fis_dir=None, feat_selc=None,noise_mirror=None, 
        dt=None, lab_lb=2.,savefile_type='pkl', no_fire=None):
        ...
        if self.stations:
            ...
            self._load_fis() # create self.fis (and self.fis_mirror) 
            self._load() # create dataframe from feature matrices
    def _load_fis(self):
        ''' Load firetive times for list of volcanos (self.stations). 
        ...
        for sta in self.stations:
            # get fires
            fl_nm=os.sep.join([self.fis_dir,sta+'_fire_ignitions.txt'])
            with open(fl_nm,'r') as fp:
                if self.no_fire:
                    self.fis[sta]=[datetimeify(ln.rstrip()) for i,ln in enumerate(fp.readlines()) if (i != self.no_fire[1] and sta is self.no_fire[0])]
                else:
                    self.fis[sta]=[datetimeify(ln.rstrip()) for i,ln in enumerate(fp.readlines())]
        ...
        if self.noise_mirror:
            ...
            for sta in self.stations:
                ...
                if True:# get period from data
                    _td=WeatherData(station=sta, data_dir=self.fis_dir)
                    _td.ti=_td.df.index[0]
                    _td.tf=_td.df.index[-1]
                if False:# get period from feature matrices available by year
                    pass
                # select random dates (don't overlap with firetive periods)
                _fis_mirror=[]
                for i in range(len(self.fis[sta])): # number of dates to create
                    _d=True
                    while _d:
                        _r=random_date(_td.ti, _td.tf)
                        # check if overlap wit firetive periods
                        _=True # non overlap
                        for te in self.fis[sta]:
                            if _r in [te-1.5*month,te+1.5*month]:
                                _=False
                        if _:
                            _d=False
                        _fis_mirror.append(_r)
                self.fis_mirror[sta]=_fis_mirror
    def _load(self):
        ...
        for sta in self.stations:
            fM=[]
            for i, te in enumerate(self.fis[sta]): 
                # FeatureSta
                feat_sta=FeaturesSta(station=sta, window=self.window, datastream=self.datastream, feat_dir=self.feat_dir, 
                    ti=te-self.dtb, tf=te+self.dtf, fis_dir=self.fis_dir, dt=self.dt, lab_lb=self.lab_lb)
    ...
    def load_fM(self, feat_dir,fl_nm,noise_mirror=None):
        ...
        # assing attribufis from file name
    ...
    def plot_svd_pcomps_noise_mirror(self):
        ...
        for i, ax in enumerate(fig.get_axes()):#[ax1,ax2,ax3]):#
            ...
            if i==0:
                for sta in self.stations:
                    ax.plot([],[], marker='.',color='r', label='firetive')
                    ax.plot([],[], marker='.',color='w', label='('+str(j)+' points)')
                    ax.plot([],[], marker='.',color='b', label='non-firetive')
...
class Feature(object):
    ...
    def load_data(self, ti=None, tf=None, exclude_dates=[]):
        if self.parent.__class__.__init__.__qualname__.split('.')[0] == 'MultiStationForecastModel':
            fMs=[]; yss=[]
    ...
    def _get_label(self, ts):
        """ Compute label vector.
            ...
            ys : list
                Label vector.
        """
        return [self.data._is_fire_in(days=self.look_forward, from_time=t) for t in pd.to_datetime(ts)]
...
if __name__ == "__main__":
    # station code dic
    sta_code={'SCA': 'Sunshine Coast Airport',
                }
    # dictionary of fire names 
    fire_dict={
                }
    if True: # FeatureSta class
        # FeatureSta
        feat_dir=r'U:\Research\fireForecasting\fires\features'
        fis_dir=r'U:\Research\fireForecasting\fires\data' 
        feat_sta=FeaturesSta(station='SCA', window=2., datastream='zsc2_T', feat_dir=feat_dir, 
            ti='2019-12-07', tf='2019-12-10', fis_dir=fis_dir)
        feat_sta.norm()
        fl_lt=r'C:\Users\aar135\codes_local_disk\volc_forecast_tl\volc_forecast_tl\models\fist\all.fts'
        feat_sta.reduce(ft_lt=fl_lt)
    if False: # FeatureMulti class
        # 
        if False: # load from server
            feat_dir=r'U:\Research\fireForecasting\fires\features'
            datadir=r'U:\Research\fireForecasting\fires\data'
        ...
        if True: # load from high speed ext hd
            feat_dir=r'E:\fireForecasting\features'
            datadir=r'E:\fireForecasting\data'
        # feature selection
        ...
        if False: # create combined feature matrix
            ...
            feat_stas=FeaturesMulti(stations=stations, window=win, datastream=datastream, feat_dir=feat_dir, 
                dtb=dtb, dtf=dtf, lab_lb=7,fis_dir=datadir, noise_mirror=True, data_dir=datadir, 
        ...
        if True: # load existing combined feature matrix 
            ...
            fl_nm='FM_2w_zsc2_rsamF_SCA-FWVZ_15dtb_0dtf.csv'
            #fl_nm='FM_2w_zsc2_T_SCA-FWVZ-KRVZ-PVV-VNSS-BELO-GOD-TBTN-MEA01_5dtb_2dtf.csv'
