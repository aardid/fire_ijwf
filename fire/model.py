...
# package imports
from fire.utilities import datetimeify, load_dataframe, save_dataframe, makedir
from fire.data import WeatherData, GeneralData
from fire.features import Feature, FeaturesSta, FeaturesMulti, _drop_features
...
class ForecastModel(object):
    """ Object for train and running forecast models.
        ...
        exclude_dates : list
            List of time windows to exclude during training. Facilitates dropping of fire 
        ...
        forecast
            Use classifier models to forecast fire likelihood.
        hires_forecast
    ...
    def _parse_data(self, data):
        self.stations=[data,]
        self.data=WeatherData(data,self,data_dir=self.data_dir, transforms=self.data_streams)
    ...
    def _model_alerts(self, t, y, threshold, ialert, dti):
        """ Compute issued alerts for model consensus.
            ...
            missed : int
                Number of fires for which an alert not issued.
            true_alert : int
                Number of fires for which an alert correctly issued.
            true_negative : int
                Equivalent number of time windows in which no alert was issued and no fire
        ...
        if len(inds) == 0:
            return 0, len(self.data.fis), 0, int(1e8), 0, 0
        ...
        tes=copy(self.data.fis)
        nes=len(self.data.fis)
        ...
        for i0,i1 in alert_windows:
            ...
            # no fires left to classify, only misclassifications now
            if len(tes) == 0:
                false_alert += 1
                continue
        ...
        # any remaining fires after alert windows have cleared must have been missed
        missed += len(tes)
    ...
    def plot_forecast(self, ys, threshold=0.75, save=None, xlim=['2019-12-01','2020-02-01']):
        ...
        for i,ax in enumerate(axs):
            ...
            # fires
            for te in self.data.fis:
                ax.axvline(te, color='k', linestyle='-', zorder=5)
            ax.axvline(te, color='k', linestyle='-', label='fire')
        ...
        for ax in axs:
            ax.fill_between([], [], [], color='y', label='fire forecast')
        axs[-1].legend()
        
        plt.savefig(save, dpi=400)
        plt.close(f)
    def _plot_hires_forecast(self, ys, save, threshold=0.75, station='SCA', nztimezone=False, alt_rsam=None, xlim=None):
    ...
    def get_performance(self, t, y, thresholds, ialert=None, dti=None):
        ...
        if not os.path.isfile(label_file):
            ys=np.array([self.data._is_fire_in(days=self.look_forward, from_time=ti) for ti in pd.to_datetime(t)])
        ...
        for j,threshold in enumerate(thresholds):
            if threshold == 0:
                FP[j]=int(1e8); dur[j]=1.; TP[j]=len(self.data.fis); TN[j]=1
            else:
...
class MultiStationForecastModel(ForecastModel):
    def __init__(self, *args, **kwargs):
        super(MultiStationForecastModel,self).__init__(*args, **kwargs)
    def _parse_data(self, data):
        self.stations=list(data.keys())
        datas=[]
        for station in self.stations:
            gd=WeatherData(station,self,data_dir=self.data_dir,transforms=self.data_streams)
            datas.append(gd)
...
class CombinedModel(object):
    ...
    dtf : datetime.timedelta
        Length of look-forward.
    dtb : float
        Days looking 'back' from fire times (for each station)
    dt : datetime.timedelta
        Length between data samples (10 minutes).
    dto : datetime.timedelta
        Length of non-overlapping section of window.
    ...
    fM : pandas.DataFrame
        Feature matrix of combined time series for multiple stations, for periods around their fire times.
        Periods are define by 'dtb' and 'dtf'. fire matrix sections are concatenated. Record of labes and times
        are keept in 'ys'. 
    ys  :   pandas.DataFrame
    ...
    noise_mirror    :   Boolean
        Generate a mirror feature matrix with exact same dimentions as fM (and ys) but for random non-fire times.
        Seed is set on  
    tes : dictionary
        Dictionary of fire times (multiple stations). 
        Keys are stations name and values are list of fire times.
    savefile_type : str
        Extension denoting file format for save/load. Options are csv, pkl (Python pickle) or hdf.
    no_erup : list of two 
        Remove fire from trainning. Need to specified station and number of fire (e.g., ['SCA',4]; fire number, as 4, start counting from 0)
    ...
    def __init__(self, data, window=2., overlap=.75, datastream=None, feat_dir=None, 
        dtb=180., dtf=2., fis_dir=None, feat_selc=None,noise_mirror=None,data_dir=None, model_dir=None,
        ...
        #
        #self._load_tes(fis_dir) # create self.fis (and self.fis_mirror) 
    ...
    def _parse_data(self, data):
        if type(data) is dict:
            ...
            self.stations=[data,]
            self.data={data:{'seismic':WeatherData(data,data_dir=self.data_dir)}}
        else:
            self.stations=data
            self.data=dict([(d,{'seismic':WeatherData(d,data_dir=self.data_dir)}) for d in data])
    ...
    def _load_feat(self):
        ...
        for i,datastream in enumerate(self.data_streams):
            ...
            if not os.path.isfile(os.sep.join([self.feat_dir,fl_nm])):
                ...
                if self.no_erup:
                    print('fire not considered:\t'+self.no_erup[0]+'\t'+str(self.no_erup[1]))
                feat_stas=FeaturesMulti(stations=self.stations, window=self.window, datastream=datastream, feat_dir=self.feat_dir, 
                    dtb=self.dtb.days, dtf=self.dtf.days, lab_lb=self.lab_lb,fis_dir=self.fis_dir, feat_selc=self.feat_selc, 
        ...
        #     # concatenate with fire dataframe FM
...
if __name__ == "__main__":
    fis_dir=r'U:\Research\EruptionForecasting\fires\fire_rep\data' 
    feat_dir=r'U:\Research\EruptionForecasting\fires\fire_rep\features'
    fl_lt=r'C:\Users\aar135\codes_local_disk\volc_forecast_tl\volc_forecast_tl\models\test\all.fts'
    if False: # ForecastModel class
        #
        data_streams=['T','DP','RH','V','Theta','KBDI','DF','VPD','FFDI','FFMC','Grass FFMC'],#'zsc2_DF','zsc2_','zsc2_']
        fm=ForecastModel(station='SCA', ti='2000-01-01', tf='2021-08-25', window=2., overlap=0.75, 
            look_forward=2., data_streams=data_streams, root='test', feature_dir=feat_dir, 
                data_dir=fis_dir,savefile_type='pkl')
        # drop features 
        ...
        # train
        #te=fm.data.fis[-1]
        fm.train(ti='2000-01-01', tf='2021-08-25', drop_features=drop_features, #exclude_dates=[[te-month/2,te+month/2],], 
            retrain=True, n_jobs=1, Nfts=10, Ncl=50) #  use_only_features=use_only_features, exclude_dates=[[te-month,te+month],]
        asdf
        # forecast
    ...
    if False: # TrainModelMulti class
        ...
        ## (1) Create model
        if True:
            data_streams=['T','DP','RH','V','Theta','KBDI','DF','VPD','FFDI','FFMC','Grass FFMC'],
            stations=['SCA']#,'KRVZ']
            dtb=60 # looking back from fire times
            dtf=0  # looking forward from fire times
            win=2.   # window length
            lab_lb=4.# days to label as fire before the fire times 
            #
            root_dir=r'U:\Research\fireForecasting\fires'
            root='FM_'+str(int(win))+'w_'+'-'.join(datastream)+'_'+'-'.join(stations)+'_'+str(dtb)+'dtb_'+str(dtf)+'dtf'
            #
            fm0=MultiStationForecastModel(stations=stations,window=win, overlap=0.75, dtb=dtb, dtf=dtf, datastream=datastream,
                root_dir=root_dir,root=root,feat_dir=feat_dir, data_dir=fis_dir,#feat_selc=fl_lt, 
