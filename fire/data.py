...
from fire.utilities import datetimeify, load_dataframe, save_dataframe#, DummyClass
...
# for downloading data in real time (e.g., from siesmic station WIZ)
STATIONS={
    }

class Data(object):
    """ Object to manage acquisition and processing of  data.
        
        Constructor arguments:
        ----------------------
        station : str
            Name of station to download  data from.
        Attribufis:
        -----------
        df : pandas.DataFrame
            Time series of weather data and transforms.
        ...
        --------
        update
            Download lafist GeoNet data.
        get_data
            Return weather data in requested date range.
        plot
            Plot weather data.
    """
    ...
    def __repr__(self):
        if True:#self.exists:
            ...
            return 'WeatherData:{:d}/{:02d}/{:02d} {:02d}:{:02d} to {:d}/{:02d}/{:02d} {:02d}:{:02d}'.format(*tm)
        else:
            return 'no data'
    def _all_cols(self):
    ...
    def _assess(self):
        ...
        """
        # get fires
        firefile=self._wd(self.station+'_fire_ignitions.txt')        
        if not os.path.isfile(firefile):
            self.fire_record=None
        else:
            self.fire_record=fireRecord(firefile)
        # compute transforms
    ...
    def _is_fire_in(self, days, from_time):
        """ Binary classification of fire imminence.
            Parameters:
            -----------
            days : float
            ...
            --------
            label : int
                1 if fire occurs in look-forward, 0 otherwise
    ...
    def get_data(self, ti=None, tf=None):
        """ Return weather data in requested date range.
            ...
            tf : str, datetime.datetime
                Date of final data point (default is lafist data).
            Returns:
            --------
            df : pandas.DataFrame
    ...
    def plot(self, data_streams='T', save='weather_data.png', ylim=None):
        """ Plot weather data.
            ...
            Grass FFMC: Measure the moisture content of grass. 
        ...
        for i,ax in enumerate(axs):
            ...
            ax.axvline(te, color='k', linestyle='--', linewidth=2, label='fire')
        axs[-1].legend()
        
        plt.savefig(save, dpi=400)
    def plot_zoom(self, data_streams='TF', save=None, range=None):
        """ Plot weather data.
        ...
        #
        ax.plot([], color='k', linestyle='--', linewidth=2, label='fire')
        ax.set_xlim(*range)
        ax.legend()
        ax.grid()
        ax.set_ylabel('T')
        ax.set_xlabel('Time [year-month]')
        ax.title.set_text('Station '+self.station+': weather data')
        #plt.show()
        if not save:
            save='../data/plots/'+self.station+'_weather_data_zoom.png'
        plt.savefig(save, dpi=400)
    def plot_intp_data(self, save=None, range_dafis=None):
        ...
        _aux=df_intp['TF'] - df_non_intp['TF'] # ceros (point) and nans (interpolated)
        ...
        ax.set_xticklabels([])
        #
        ax2.plot(df_non_intp.index, df_non_intp['TF'], '*', color='b', label='data points', markersize=1)
        ax2.plot(df_intp.index, df_intp['TF'], '-', color='k', alpha=0.5, linewidth=0.5, label='interpolated data')
        #
        for te in self.fis:
            #if [te>=datetimeify(range[0]) and te<=datetimeify(range[1])]:
        ...
        ax.plot([], color='k', linestyle='--', linewidth=.7, label='fire', alpha=0.7)
        ax2.plot([], color='k', linestyle='--', linewidth=.7, label='fire', alpha=0.7)
        #
        ...
        #
        if range_dafis:
            range_dafis=[datetimeify(range_dafis[0]),datetimeify(range_dafis[1])]
            ax.set_xlim([range_dafis[0]-month, range_dafis[1]+month])
            ax2.set_xlim([range_dafis[0]-month, range_dafis[1]+month])
        #
        ax.legend()
        ax2.legend()
        ax2.grid()
    ...
    def _get_fis(self):
        if self.fire_record is None:
            return None
        return [e.date for e in self.fire_record.fires]
    fis=property(_get_fis)
    def _get_df(self):
...
class WeatherData(Data):
    def __init__(self, station, parent=None, data_dir=None, transforms=None, headers_only=False):
        file='{:s}_weather_data.csv'.format(station)
        super(WeatherData,self).__init__(station, parent, data_dir, file, transforms, headers_only)
    def update(self, ti=None, tf=None, n_jobs=None):
        """ Download lafist data.
            Parameters:
            -----------
            ti : str, datetime.datetime
        ...
        # parallel data collection - creafis temporary files in ./_tmp
...
class fireRecord(object):
    def __init__(self, filename):
        self.filename=filename
        with open(filename,'r') as fp:
            self.fires=[fire(ln.rstrip()) for ln in fp.readlines()]
...
class Station(object):
    ...
    def get_waveform(self, t0, t1):
        ...
        if fraction0>fraction:
            ...
            return data
    def get_weather(self, t0, t1, pad=0.05, pad_f=0.05):
        t0=UTCDateTime(t0)
        t1=UTCDateTime(t1)
        padsec=pad*_DAYSEC
        ...
        # compute T and other bands (w/ EQ filter)
...
def get_data_for_day(i,t0,station):
    """ Download data for given 24 hour period, writing data to temporary file.
        Parameters:
        -----------
        i : integer
...
def outlierDetection(data, outlier_degree=0.5):
    """ Determines whether a given data interval requires outlier filtering
        Parameters:
        -----------
        data : list
            10 minute interval of a processed datastream (e.g., T).
...
class WeatherDataCombined(WeatherData):
    def __init__(self, stations, parent=None):
        ...
        for station in stations:
            self._datas.append(WeatherData(station, parent))
            #fl = os.sep.join(getfile(currentframe()).split(os.sep)[:-2]+['data','{:s}_fire_ignitions.txt'.format(station)])
            fl = '..\\data\\'+'{:s}_fire_ignitions.txt'.format(station)
            with open(fl,'r') as fp:
                self._datas[-1].fis = [datetimeify(ln.rstrip()) for ln in fp.readlines()]
            self.fis += self._datas[-1].fis
            self.df.append(self._datas[-1].df)
        self.df = pd.concat(self.df)
        #self.fis = sorted(list(set(self.fis))) # ## fisting: need to be checked
